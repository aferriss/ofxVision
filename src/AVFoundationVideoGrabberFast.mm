/*
 *  AVFoundationVideoGrabberFast.mm
 */
...
#warning Target = iOS Simulator - The AVFoundationVideoGrabberFast will not function on the iOS Simulator
#endif

//CVOpenGLESTextureCacheRef videoTextureCache = NULL;


@interface iOSVideoGrabberFast() <AVCaptureVideoDataOutputSampleBufferDelegate> {
...
@implementation iOSVideoGrabberFast
        ...
        if(IS_IOS_7_OR_LATER) {
			...
			if(!error) {
				...
				if(framerate < minFrameRate) {
					NSLog(@"iOSVideoGrabberFast: Framerate set is less than minimum. Setting to Minimum");
					framerate = minFrameRate;
				}
				if(framerate > maxFrameRate) {
					NSLog(@"iOSVideoGrabberFast: Framerate set is greater than maximum. Setting to Maximum");
				...
				NSLog(@"iOSVideoGrabberFast Init Error: %@", error);
			}
            #endif
        }
...
AVFoundationVideoGrabberFast::AVFoundationVideoGrabberFast(){
	fps		= 30;
	grabber = [iOSVideoGrabberFast alloc];
	pixels	= NULL;
    width = 0;
    height = 0;
...
AVFoundationVideoGrabberFast::~AVFoundationVideoGrabberFast(){
	ofLog(OF_LOG_VERBOSE, "AVFoundationVideoGrabberFast dtor");
	bLock = true;
	if(grabber) {
...
void AVFoundationVideoGrabberFast::switchCamera(){
	if(grabber){
...
void AVFoundationVideoGrabberFast::clear(){
...
void AVFoundationVideoGrabberFast::setCaptureRate(int capRate){
	fps = capRate;
}

bool AVFoundationVideoGrabberFast::initGrabber(int w, int h, bool bNp){
...
void AVFoundationVideoGrabberFast::startCapture(){
	[grabber startCapture];
}

void AVFoundationVideoGrabberFast::stopCapture(){
	[grabber stopCapture];
}

bool AVFoundationVideoGrabberFast::isInitialized(){
    return bIsInit;
}

void AVFoundationVideoGrabberFast::update(){
	if(bNeedsPixels){
		newFrame = false;
...
void AVFoundationVideoGrabberFast::focusOnce(){
    [grabber focusOnce];
}

void AVFoundationVideoGrabberFast::lockFocus(){
	
}

void AVFoundationVideoGrabberFast::touchFocusAt(ofPoint pt){
	[grabber touchFocusAt:CGPointMake(pt.x, pt.y)];
}

void AVFoundationVideoGrabberFast::setContinuousAutoFocus(){
	[grabber setContinuousAutoFocus];
}

void AVFoundationVideoGrabberFast::updatePixelsCB( CGImageRef & ref ){
...
bool AVFoundationVideoGrabberFast::isFrameNew() {
	return newFrame;
}

ofTexture * AVFoundationVideoGrabberFast::getTexture() {
	return &texture;
}

vector <ofVideoDevice> AVFoundationVideoGrabberFast::listDevices() {
...
void AVFoundationVideoGrabberFast::setDevice(int deviceID) {
	[grabber setDevice:deviceID];
	device = deviceID;
}

bool AVFoundationVideoGrabberFast::setPixelFormat(ofPixelFormat PixelFormat) {
...
ofPixelFormat AVFoundationVideoGrabberFast::getPixelFormat() {
	if(internalGlDataType == GL_RGB){
...
CVPixelBufferRef AVFoundationVideoGrabberFast::getRawVideoRef() {
    return [grabber getPixelBuffer];
}
